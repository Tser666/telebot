      ...
      <summary className="cursor-pointer list-none">
        <Card className="transition-colors group-open:border-primary/40">
          <CardHeader className="pb-3">
            ...
            <CardDescription>
              回复任意 Telegram 消息后发送 <CommandBadge className="mx-1">{cmdPrefix}ai 你的问题</CommandBadge>，worker 会把回答编辑回命令消息。
            </CardDescription>
          </CardHeader>
          <CardContent className="hidden space-y-3 text-sm group-open:block">
            <ol className="list-decimal space-y-1.5 pl-5 text-muted-foreground">
              <li>
                前缀来自系统设置里的命令前缀 <CommandBadge>{cmdPrefix}</CommandBadge>，不是固定的 <CommandBadge>/</CommandBadge>。
              </li>
              <li>worker 只拦截自己发出的 outgoing 命令，别人发送同样命令不会触发。</li>
              <li>
                被回复消息正文加上命令后的问题会拼成 user prompt，模板里的 <code>system_prompt</code> 负责设定回答风格。
              </li>
              <li>
                返回时编辑你的命令消息；末尾会附上模型名、in/out tokens，自动路由会额外标出决策原因。
              </li>
              <li>完整配置路径是先添加模型提供商，再创建 type=ai 的命令模板，最后到账号详情启用命令。</li>
            </ol>
            <div className="rounded-md border px-3 py-2 text-xs alert-info">
              ...
              <p className="mt-1">
                ...
                <CommandBadge>{cmdPrefix}ai image 画一张极简图标</CommandBadge>；也可以新建直接命令
